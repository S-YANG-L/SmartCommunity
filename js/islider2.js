...
iSlider = function () {
  ...
  iSlider.prototype._renderItem = function (el, i) {
    ...
    var insertImg = function () {
      html = item.height / item.width > self.ratio ? '<img height="' + self.height + '" src="' + item.filePath + '">' : '<img width="' + self.width + '" src="' + item.filePath + '">';
      el.innerHTML = html;
    };
    ...
    if (this.type === 'pic') {
      if (!this.isOverspread) {
        ...
        el.style.background = 'url(' + item.filePath + ') 50% 50% no-repeat';
        el.style.backgroundSize = 'cover';
      }
    } else if (this.type === 'dom') {
      el.innerHTML = item.filePath;
    }
  };
  /**
   * render list html
  ...
  iSlider.prototype._preloadImg = function (dataIndex) {
    ...
    var loadImg = function (index) {
      if (index > -1 && !self.data[index].loaded) {
        ...
        preloadImg.src = self.data[index].filePath;
